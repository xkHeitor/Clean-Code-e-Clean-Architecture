...
export default class StockEntryRepositoryDatabase implements StockEntryRepository {
    ...
    async clean(): Promise<void> {
        await this.connection.query("delete from ccca.stock_entry", null);
    }

    async save(stockEntry: StockEntry): Promise<void> {
        await this.connection.query("insert into ccca.stock_entry (id_item, operation, quantity) values ($1, $2, $3)", 
        [ stockEntry.idItem, stockEntry.operation, stockEntry.quantity ]);
    }
    
    async getAll(idItem: number): Promise<StockEntry[]> {
        const stockEntriesData = await this.connection.query("select * from ccca.stock_entry where id_item = $1", 
        [ idItem ]);
